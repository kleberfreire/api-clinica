...
import { formateDateBR, formateDateEN } from '../../../functions/formatDate'
...
export default function AddressForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await api.get(`/api/consulta/findOne/${id}`)
      console.log('id consulta', result.data)
      ...
      if(result.status === 200) {
        await setObservacao(result.data.observacao)
        await setDate(formateDateEN(result.data.data))
        await setSelectCliente(result.data.cliente.id)
        await setSelectMedico(result.data.medico.id)
        await setHora(result.data.hora)
      } 
    }   
  ...
  const handleSubmit = async() => {
    ...
    const response = await api.post('/api/consulta/update/'+ id,data) 
  ...
  return (
    ...
    <React.Fragment>
      <Typography variant="h6" gutterBottom>
        Fomulario de alteração de consulta
      </Typography>
